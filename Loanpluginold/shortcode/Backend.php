...
function create_financial_calculator_post_type() {
    ...
    $args = array(
        ...
        'hierarchical' => false,
        'menu_position' => 3,
        'supports' => array('title'), 'menu_icon' => 'dashicons-calculator',
    );
...
function financial_calculator_add_meta_boxes() {
    ...
    add_meta_box('interest_rate_type', 'Interest Rate Type', 'interest_rate_type_callback', 'financial_calculator', 'normal', 'default');
}

add_action('add_meta_boxes', 'financial_calculator_add_meta_boxes');

function interest_rate_type_callback($post) {
    wp_nonce_field('interest_rate_type_meta_box', 'interest_rate_type_nonce');
    $interest_rate_value = get_post_meta($post->ID, 'interest_rate_value', true);
    print_r($interest_rate_value);
    if (!$interest_rate_value) {
        $interest_rate_value = 0; // default value
    }
    ?>
    <label for="interest_rate_value">Interest Rate :</label>
    <input type="range" name="interest_rate_value" class="slider" id="interest_rate_value" min="0.2" max="100" step="0.1" value="<?php echo esc_attr($interest_rate_value); ?>">
    <span id="interest_rate_display"><?php echo esc_html($interest_rate_value); ?></span>
    <script>
        jQuery(document).ready(function ($) {

            $('#interest_rate_value').on('input', function () {
                $('#interest_rate_display').text($(this).val());
            });
        });
    </script>
    <style>
...
function financial_type_callback($post) {
    ...
    <select name="financial_type" id="financial_type">
        <option value="hcstc_loan" <?php selected($financial_type, 'hcstc_loan'); ?>>Personal Loan</option>
...
function financial_calculator_save_meta_boxes($post_id) {
    ...
    if (isset($_POST['interest_rate_value'])) {
        update_post_meta($post_id, 'interest_rate_value', sanitize_text_field($_POST['interest_rate_value']));
    }
...
function remove_view_action_link($actions, $post) {
    // Check for your custom post type
    if ($post->post_type == 'financial_calculator') {
        // Remove the view action link
        unset($actions['view']);
...
add_filter('post_row_actions', 'remove_view_action_link', 10, 2);
