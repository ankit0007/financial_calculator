(function ($) {
    ...
    $(document).on("input", '.diposit,.mortgagebalance input,.validation,.amount input,.Amount input,.paymentamount input,.Fee input', function (evt) {
        var self = $(this);
    ...
    $(document).on('click', '.btns input', function () {
        ...
        if (Nclass.find('.totalpay span').html() == 'NaN' || Nclass.find('.permonthpay span').html() == 'NaN')
        {
            //alert('hi');
            Nclass.find('.totalpay span,.permonthpay span').html('0.0')
        }
    });
})(jQuery);
...
function mortgage_payments(Nclass) {
    ...
    let period = parseFloat(Nclass.find('.LoanTerm select').val());
    let apr = parseFloat(Nclass.find('.APR select').val());
...
function balance_transfer(Nclass) {
    ...
    let aprOffset = parseFloat(Nclass.find('.aproffset select').val());
    let apr = parseFloat(Nclass.find('.APR select').val());
    let paymentAmount = parseFloat(Nclass.find('.paymentamount input').val());
    let fee = parseFloat(Nclass.find('.Fee input').val());

    
    let aprInput = (apr / 100) / 12;
    let offsetInterest = 0;
    balance += fee;
    let y = balance;
    let array = [];
    let term = 0;
    while (term < aprOffset && y > 0) {
      y -= paymentAmount;
      let x = y * offsetInterest;
      y += x;
      array.push(x);
      term++;
    }
    ...
    let interest = array.reduce((a, b) => a + b, 0);
    let totalPay = interest + balance;
    totalPay=Math.round(totalPay).toFixed(2);
    paymentAmount=Math.round(paymentAmount).toFixed(2);
    interest=Math.round(interest).toFixed(2);
    // Message
    let message = `To clear your balance of ${balance} with a monthly payment of ${paymentAmount} at ${offsetInterest}% apr for the first ${aprOffset} months followed by ${apr}% apr you would need to pay a total of ${totalPay} over a period of ${term} months. Interest is ${interest}`;
    console.log(message);
    Nclass.find('.extraresult').removeClass('active');
    Nclass.find('.extraresult').addClass('active');
    Nclass.find('.extraresult').html(message);
    Nclass.find('.totalpay span').html((totalPay));
    Nclass.find('.permonthpay span').html(paymentAmount); 
    return message;
}
...
function hcstc_loan_calc(Nclass) {
    let loanamount = parseFloat(Nclass.find('.amount input').val());
    let period = parseInt(Nclass.find('.LoanTerm select').val());
    let apr = parseFloat(Nclass.find('.APR select').val());
    // Creating inputs for calculator
    const principal = loanamount / period;
    ...
    let message;
    if (rule_check === 0) {
        message = `hello A loan of £${loanamount} over ${period} months at ${apr}% apr will be a total of £${totalpay} at a monthly payment of £${monthlypay}`;
